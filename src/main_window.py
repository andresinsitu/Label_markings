...
class MainWindow(QMainWindow):
    ...
    def __init__(self, workdir: str):
        ...
        self.ls_autoseg_value.setText("Autoseg mask opacity: 15%")
    ...
    def on_ls_threshold_slider_change(self, value: int):
        self.ls_threshold_value.setText(f"Autoseg threshold: {90+value*5}")
        name = f"{self._image_stems[self._curr_id]}.png"
        image_path = self._image_dir / name
        autoseg_path = self._autoseg_dir / name
        self._graphics_view.set_autoseg_threshold(str(image_path), str(autoseg_path),90+value*5)
